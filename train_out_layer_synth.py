...
def get_args() -> Namespace:
    ...
    parser.add_argument(
        ...
        metavar="DATA_FOLDER",
        type=str,
        help="Folder containing train and validation.",
    )
    parser.add_argument(
        "test_folder",
        metavar="TEST_FOLDER",
...
def get_loaders(processor, args) -> Tuple[DataLoader, DataLoader]:
    with open(os.path.join(args.data_folder, "train_ids.json"), "r") as f:
        train_ids = set(map(int, json.load(f)))

    with open(os.path.join(args.data_folder, "valid_ids.json"), "r") as f:
        val_ids = set(map(int, json.load(f)))

    train_dataset = CustomDataset(args.data_folder, processor, ids=train_ids)
    val_dataset = CustomDataset(args.data_folder, processor, ids=val_ids)

    assert len(train_dataset) == len(
        train_ids
    ), f"Expected {len(train_ids)} samples in the training dataset, but got {len(train_dataset)}."
    assert len(val_dataset) == len(
        val_ids
    ), f"Expected {len(val_ids)} samples in the validation dataset, but got {len(val_dataset)}."
